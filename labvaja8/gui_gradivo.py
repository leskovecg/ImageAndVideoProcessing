    ...
    def execHough(self):
        ...
        if self.iImage_edge is None:
            ...
            self.view_hough.imshow(
                self.iImage_acc, rangeF, rangeR, "\u03C6 [deg]", "r [px]"
            )
...
def houghTransform2D2P(iImage, stepR, stepF):
    ...
    oAcc = np.zeros((rangeR.size, rangeFrad.size))
    
    # sprehodimo se po binarni sliki
    for y in range(Y):
        for x in range(X):
            # če je to točka ki pripada robu na sliki, povečamo vrednost akumulatorja za določene r-je in fi-je 
            if iImage[y, x]:
                
                for f_idx, fi in enumerate(rangeFrad):
                    # za dani kot in točko (x,y) poiščemo pripadajoči r 
                    r = x*np.cos(fi) + y*np.sin(fi)
                    
                    if r >= rangeR[0] and r <= rangeR[-1]:
