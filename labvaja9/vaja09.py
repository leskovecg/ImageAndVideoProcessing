...
half_min_middle_widget_width = min_middle_widget_width / 2
...
class ui_MiddleLeftPanel(QtWidgets.QWidget):
    def __init__(self, parent=None, pts_list=[]):
        ...
        # load image
        self.load_button_ref = QtWidgets.QPushButton("Naloži sliko...", self)
        # list of points
...
class ui_MiddleRightPanel(QtWidgets.QWidget):
    def __init__(self, parent=None, pts_list=[]):
        ...
        # load image
        self.load_button_input = QtWidgets.QPushButton("Naloži sliko...", self)
        # list of points
        self.listWidget = QtWidgets.QListWidget(self)
        self.listWidget.setSelectionMode(QtWidgets.QAbstractItemView.SingleSelection)
        # delete item from list
...
class ui_MiddleBottomPanel(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        # ----------------------------------------------------------
        ## razlika med referenco in poravnano vhodno sliko
        # group_diff_image = QtWidgets.QGroupBox(
        #     "Razlika med referenco in poravnano vhodno sliko", self
        # )
...
class mainWindow(QtWidgets.QWidget):
    ...
    def register_images(self):
        ...
        if self.upperPanel.first_option.isChecked():
            ...
            warning("Pri poravnavi je prišlo do težave.")
            return
    ...
    def get_diff_and_chess(self):
        try:
            ...
            warning("Pri izracunu sahovnice prišlo do težave.")
            return
...
def affineRegistration(iType, rCP, iCP, iImage):
    ...
    # pripravimo si vektorje koordinat:
    X = rCP[:,0] # : --> vse vrstice in prvi stolpec
    Y = rCP[:,1] # : --> vse vrstice in drugi stolpec
    
    U = iCP[:,0]
    V = iCP[:,1]
    
    # afina interp. poravnava
    if iType == 'interpolation':
        XY = np.ones((3, X.size), dtype = float)
        UV = np.ones((3, U.size), dtype = float)
        # ref. matrika
        XY[0, :] = X
        XY[1, :] = Y
        # vhodna matrika
        UV[0, :] = U
        UV[1, :] = V
        # determinanta matrike XY mora biti različna od nič, linalg -- linear algebra knjižnica 
        if np.linalg.det(XY) != 0:
            oT = UV @ np.linalg.inv(XY)
        else: 
            raise ValueError('matrika ni obrnljiva')
            
    elif iType == 'approximation':
        #pass --> ukaz pomeni preskočiti to kodo
        # določimo število elementov v vektorju
        K = X.size
        # izračunamo elemente matrike
        x = X.sum() / K
        xx = (X*X).sum() / K
        y = Y.sum() / K
        yy = (Y*Y).sum() / K
    ...
    # če uporabnik ne izbere nobenih točk (brez poravnave)
    if iCP.size:
        pts = np.ones((3, U.size), dtype = float)
        pts[0, :] = U
        pts[1, :] = V
        oCP = np.linalg.inv(oT) @ pts
        oCP = oCP[:2, :].T
    
    # afina preslikava slik
...
def computeError(rCP, iCP, oCP, rImage, iImage, oImage, iArea):
    ...
    y_koncni = y_zacetni + iArea[3]
    x_koncni = x_zacetni + iArea[2]
    ...
    for j in range(0, J):
        for i in range(0, I):
            if (j >= y_zacetni and j <= y_koncni) and (i >= x_zacetni and i <= x_koncni):
                MSE1 += (rImage[j,i] - iImage[j,i])**2
                MSE2 += (rImage[j,i] - oImage[j,i])**2
